 ...
 * static String dburl="jdbc:mysql://localhost:3306/bankappdb"; private static
 * String username = "root"; private static String password = "root";
 * 
 * static Connection connection; static final Logger logger =
 * Logger.getLogger(DbUtil.class);
 * 
 * public static Connection getConnection() { try {
 * Class.forName(driverClassName); if (connection == null) { connection =
 ...
 * 
 * public static void commit() { try { if (connection != null) {
 * connection.commit(); } } catch (SQLException e) {
 * logger.error("SQLException", e); }
 * 
 * }
 * 
 * public static void rollback() { try { if (connection != null)
...
/*
 * static { try { File file = new File("dbCOnfig.properties"); FileReader read =
 * new FileReader(file);
 * 
 * Properties p = new Properties(); p.load(read); read.close();
 * 
 * driverClassName = p.getProperty("driverClassName"); dburl =
 * p.getProperty("dburl"); username = p.getProperty("username"); password =
